  ...
  let rec exec_call f args =
    ...
    let rec eval_expr = function
      ...
      | Binop(Add, e1, e2) ->
        ...
        VInt (v1 + v2)
        (* VInt (vint (eval_expr e1) + vint (eval_expr e2)) *) 
        
        let local_env = Hashtbl.create 16
        let global_env = Hashtbl.create 16

        Var x -> 
          if Hashtbl.mem local_env x then
            Hashtbl find local_env x
          else
            Hashtbl.find global_env x
