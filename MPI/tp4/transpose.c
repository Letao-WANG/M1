...
int main(int argc, char *argv[]) {
  ...
  // printf("size_real: %d", size_real);
  // printf("MPI_DOUBLE: %d", sizeof(MPI_DOUBLE));
  // printf("size_displacement: %d", size_displacement);
  MPI_Type_create_hvector(nb_lines,1,size_displacement,type_column,&type_transpose);
  ...
  if (rank == 0) {
    ...
    MPI_Send(a,nb_lines,MPI_DOUBLE,1,tag,MPI_COMM_WORLD);

  } else {
    /* Receive in matrix AT with type_transpose */
    // MPI_Recv(at,1,type_transpose,0,tag,MPI_COMM_WORLD,&status);
    ...
    for (iterl=0; iterl<10;iterl++) {
      printf("%4.f ", res[iterl]);
    }
    printf("\n");
  }
